...
def test_cpu_instruction_bvc_overflow_flag_false(setup):
    "Test BVC when overflow flag is False"
    computer = setup
    computer.reset()
    ...
    # BVC to relative address -2 (the ASL instruction)
    computer.memory.write(0x01, 0x50)
...
def test_cpu_instruction_bvc_overflow_flag_true(setup):
    "Test BVC when overflow flag is True"
    computer = setup
    computer.reset()
