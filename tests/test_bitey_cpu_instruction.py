...
def test_cpu_instruction_init():
    i = Instruction("LDA", 173, "Load Accumulator with Memory")
    assert i.name == "LDA"
    assert i.opcode == 173
...
def test_cpu_instructions_init():
    i1 = Instruction("LDA", 173, "Load Accumulator with Memory")
    i2 = Instruction("STA", 141, "Store Accumulator in Memory")
